...
function promotion_boxYTShortcode($atts,$contentC,$module_id,$id,$database){
	...
	if($module_id != "0"){		
        ...
        if($atts->type=="border")
        {
        	$background = 'border:' . $atts->promotion_background.' '.$atts->border.'px solid; background:#fff;';					
			$css .= '#'.$id.' .border{'.$background.'}';
        }else if($atts->type =="background-border")
        {
        	$background = 'border:' . $atts->border_color.' '.$atts->border.'px solid; background:'.$atts->promotion_background.';';
	...
	/*--------------------------------------------------*/
        ...
        $return .= "<div class='cta-content'>" . $title ."<div></div></div>";
