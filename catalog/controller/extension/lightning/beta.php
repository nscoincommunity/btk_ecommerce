...
if(!empty($Ma['bz']))$Mti=array();$Mfv=@disk_free_space(DIR_CACHE);if($Mfv<10)$Mfv=2*500*1024*1024;$Miq=true;$Mip=-1;$Mus=substr(DIR_SYSTEM,0,-7)."vqmod/checked.cache";if(file_exists($Mus)&&filesize($Mus)>500000)@unlink($Mus);function Wbu(){return new light_db(Ws());}
class light_db{public$driver;public function __construct($driver){global$Man,$Mfg;$Man=DIR_CACHE."lightning/beta";$Mfg=$GLOBALS['Ma']['ci']*60;$this->driver=$driver;}
private function Wbw($Mis,$Mit=false){$Mis=str_replace('`','',$Mis);if(!$Mis)return 0;if($Mis!=strtolower($Mis))return 0;static$Miu;global$Man;if(!$Mit and!empty($Miu[$Mis]))return$Miu[$Mis];$Miv=$Man."/qwert";$Map=$Miv."/$Mis";if($Mit){Waw($Miv);if(!file_exists($Map))file_put_contents($Map,'',LOCK_EX);touch($Map);$Miu[$Mis]=time();return$Miu[$Mis];}
...
if(!$Mjs){Wbb($Mhj,serialize($Mhp->rows));if(stripos($Mhj," RAND()")){global$Mfg;Wbe($Mhj,time()-$Mfg+60);}}}}
...
global$Mir;$Mi_=$Mir->query("SELECT page FROM ".DB_PREFIX."lightning_product_to_page WHERE product_id IN(".implode(",",$Miy).")")->rows;if(!$Mi_)return false;global$Man;$Mja=$Man;foreach($Mi_ as$Mfd){$Mfd=$Mfd["page"];$Man=DIR_CACHE."lightning/alpha";Wb_($Mfd);$Man=DIR_CACHE."lightning/tetha";Wb_($Mfd);$Man=DIR_CACHE."lightning/gamma";Wb_($Mfd);}
...
define('Wcw',"/meters");function Wcr(){global$Mke,$Mkf,$Man,$Mfg,$Miq;if(!$Mke)$Mke=0;$Mkf[$Mke][0]=$Man;$Mkf[$Mke][1]=$Mfg;$Mkf[$Mke][2]=$Miq;$Mke++;}
function Wcs(){global$Mke,$Mkf,$Man,$Mfg,$Miq;$Mke--;$Man=$Mkf[$Mke][0];$Mfg=$Mkf[$Mke][1];$Miq=$Mkf[$Mke][2];}
function Wad($Mkg=false,$Mkh=false){global$Man;$Map=$Man.Wcw;if(file_exists($Map)){$Mbq=explode(',',Wct($Map));if(!$Mkg and!$Mkh)return$Mbq;$Mki=$Mbq[0];$Mbj=$Mbq[1];}else{$Mki=0;$Mbj=0;}
...
function Wbb($Mgj,$Mbq,$Mkk=false){global$Man,$Miq;if(Waw($Man))if($Miq)file_put_contents($Man.Wcw,"0,0",LOCK_EX);global$Mfv,$Mb;if($Mfv<$Mb)return;$Mao=md5($Mgj);$Miv=$Man."/".substr($Mao,0,2);if($Mkk)Wcu($Miv);$Map=$Miv."/c".substr($Mao,2);if($Miq){$Mki=0;if(file_exists($Map))$Mbj=strlen($Mbq)-filesize($Map);else{$Mbj=strlen($Mbq);$Mki=1;}
Wad($Mki,$Mbj);}
Waw($Miv);file_put_contents($Map,$Mbq,LOCK_EX);return$Map;}
...
function Wae($Mgj){global$Man;$Mao=md5($Mgj);$Miv=$Man."/".substr($Mao,0,2);$Map=$Miv."/c".substr($Mao,2);if(!Whg($Map))return false;return(filemtime($Map));}
function Wbe($Mgj,$Mbt,$Mkm=false){global$Man;$Mao=md5($Mgj);$Miv=$Man."/".substr($Mao,0,2);Waw($Miv);$Map=$Miv."/c".substr($Mao,2);if(!Whg($Map)){if(!$Mkm)return;file_put_contents($Map,'');global$Miq;if($Miq)Wad(1,0);}
@touch($Map,$Mbt);}
function Wbd($Mgj,$Mkn=false){global$Mjq,$Man,$Mfg;$Mjq=false;$Mao=md5($Mgj);$Miv=$Man."/".substr($Mao,0,2);$Map=$Miv."/c".substr($Mao,2);if(!$Mkn)Wcu($Miv);if(!Whg($Map))return false;$Mjq=filemtime($Map);static$Mmk;if(!$Mmk)$Mmk=time();$Mbt=$Mmk-$Mfg;if(!$Mkn and$Mjq<$Mbt){unlink($Map);return false;}
$Mbq=file_get_contents($Map);return$Mbq;}
function Wcu($Miv){global$Mfg,$Miq;if(!$Mfg)return;static$Mka;if(!empty($Mka[$Miv]))return;$Mka[$Miv]=true;if(!Whg($Miv))return;$Mbt=time();if(filemtime($Miv)<$Mbt-10*60){touch($Miv);$Mbt-=$Mfg;$Mki=0;$Mbj=0;if(substr($Miv,-1)=="/")$Miv=substr($Miv,0,-1);$Mko=glob($Miv."/c*");if($Mko)foreach($Mko as$Mj_)if(Whg($Mj_)&&filemtime($Mj_)<$Mbt){if($Miq)$Mbj+=filesize($Mj_);@unlink($Mj_);$Mki++;}
if($Miq)Wad(-$Mki,-$Mbj);}}
function Wag($Mgj=false){global$Man,$Miq;if($Mgj){$Mao=md5($Mgj);$Miv=$Man."/".substr($Mao,0,2);$Map=$Miv."/c".substr($Mao,2);if(Whg($Map)){Wad(-1,filesize($Map));unlink($Map);}
return;}
foreach(glob($Man."/*")as$Miv){if(is_dir($Miv)){foreach(glob($Miv."/*")as$Mj_)if(Whg($Mj_))@unlink($Mj_);rmdir($Miv);}else if(Whg($Miv))@unlink($Miv);}
if($Miq and Whg($Man.Wcw))@unlink($Man.Wcw);}
function Wb_($Mkp){global$Man;$Map=$Man."/".$Mkp;if(Whg($Map)){Wad(-1,-filesize($Map));unlink($Map);}}
...
static$Mlx;if(!$Mlx)$Mlx=time()-60*60;static$Mer;if(isset($Mer[$Mjc]))return$Mer[$Mjc];$Mhp=Wbd($Mjc,true);if($Mjq<$Mlx+rand(0,10*60))return-1;if(!is_numeric($Mhp))return-1;$Mer[$Mjc]=$Mhp;Wci($Mjc,$Mhp);$Mjc=false;return$Mhp;}
function Wcc($Mhp){global$Mjc;if(!isset($Mhp->row["total"])||count($Mhp->rows)>1){$Mjc=false;return;}
Wbb($Mjc,$Mhp->row["total"]);Wci($Mjc,$Mhp->row["total"]);$Mjc=false;}
...
return copy($Muh,$Mui);}
function Whv($Mww){$log="lightning";if($Maf=strpos($Mww,":")){$log.='_'.strtolower(substr($Mww,0,$Maf));$Mww=substr($Mww,$Maf+2);}
