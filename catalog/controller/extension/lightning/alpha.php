...
function Wq($Mbo=false){global$Mf,$Mz,$Maq,$Man,$Mc;if(!Wdr){header("HTTP/1.1 200 OK",true,200);header("Status: 200");if(function_exists("header_remove")){header_remove("Content-Encoding");header_remove("Set-Cookie");header_remove("Location");}else{header("Content-Encoding: none");header("Set-Cookie:");}
if(function_exists("http_response_code"))http_response_code(200);}else if(!$Mbo)if(!file_exists(Wa.'ar'))Wfy(Wa.'aq');if(!$Mf){if($Mbo and!Wdr){if($Mz=="gen"&&!$Maq['id']){$Man=DIR_CACHE."lightning/alpha";$Mbp=Wad();$Mbq["pages"]=$Mbp[0];$Mbq["psize"]=Wac($Mbp[1]);global$Mtf;if(!$Mtf){$Mbq["width"]=round(($Maq['Mha']-count($Maq['Mbz']))/$Maq['Mha']*200);$Mbq["width"].="px";$Mbq["percent"]=round(($Maq['Mha']-count($Maq['Mbz']))/$Maq['Mha']*100);if(!empty($Maq["subgen"])){$Mbq["width"]="200px";$Mbq["percent"]=100;}}
...
global$Mtd,$Maq,$M_,$Man,$Mk,$My,$Ml,$Mbr,$Ma,$Mtf,$Mte;$Mtf='';while(file_exists(Wa.'bb'.$Mtf))$Mtf++;$Mtf--;if(!$Mtf)$Mtf='';$Maq=unserialize(file_get_contents(Wa.'bb'.$Mtf));if(!Wdr&&$Maq['id']!=$M_){$Mbq["gen"]=false;$Mbq["md"]=array();global$Mz;if($Mz=="gens")die(json_encode($Mbq));else die("false");}
require_once"beta.php";$Man=DIR_CACHE."lightning/alpha";if(!$Mbr)$Mbr=$Mk;$Mk=false;foreach($Maq['Mbz']as$Mbs=>$Mbe){$Mbe=str_replace("&amp;","&",$Mbe);if($Mte and!$Maq['id']and!$Mtf){$Mbv=round(($Maq['Mha']-count($Maq['Mbz']))/$Maq['Mha']*100);file_put_contents(Wa.'ar',$Mbv,LOCK_EX);}
unset($Maq['Mbz'][$Mbs]);$Mbt=Wae(Wm($Mbe))-Wy(Wm($Mbe));$Mk=$Mbe;if($Mbt<=$Ml)break;if(!empty($Maq['ek'])&&$Mtf<$Mtd){$Mao=md5(Wm($Mk));$Mcd=DIR_CACHE."lightning/gamma"."/".substr($Mao,0,2)."/c".substr($Mao,2);if(Waf($Mcd,$Mk)){Whr(Wa.'bb'.$Mtf,$Maq);$Mer=$Maq;if(!Wn(true))return;$Maq=$Mer;}}
...
function Waf($Map,$Mtg=false){global$Man,$Maq,$M_,$Ml,$Mm,$Mtf,$Mtd;if($Mtg&&$Mtf>=$Mtd)return;$Man=DIR_CACHE."lightning/alpha";require_once"beta.php";if($Mtg)$Mth=$Maq;$Maq['Map']=$Map;$Maq['id']=$M_;$Maq['Mca']=time();$Maq['Mig']=$Mtg;if(strpos($Map,"//")!==false){$Maq['Map']=false;$Mbz=array($Map);}else if(file_exists($Map))$Mbz=unserialize(file_get_contents($Map));if(empty($Mbz))$Mbz=array();$Mb_=array();$Mca=time();foreach($Mbz as$Mcb=>$Mbe){$Mbt=Wae(Wm($Mbe));if($Mbt<$Mm){$Mb_[]=$Mbe;unset($Mbz[$Mcb]);continue;}
if($Mbe==$Map){Wag(Wm($Mbe));continue;}
$Mbt=$Mbt-Wy(Wm($Mbe));if($Mtg&&$Mbt>$Ml){$Man=DIR_CACHE."lightning/gamma";$Mto=Wae(Wm($Mbe));if($Mto>$Ml){unset($Mbz[$Mcb]);}
$Man=DIR_CACHE."lightning/alpha";}
if($Mbt<$Mca)$Mca=$Mbt;}
...
$Mdo=lc('dh');$Mqp="";$Mdp="";$Mdq="";if(lc('ef')and!Wey('dw')){$Mgw=lc('ef');$Mdq.="<script>window.performance.mark('Start JS Execution');</script>";$Mdr=Wat("<script","</script>",$Mg);foreach($Mdr as$Mcz)if(strpos($Mcz,"<!--")){$Mne=str_replace("<!--",'',$Mcz);$Mne=str_replace("-->",'',$Mne);$Mg=str_replace($Mcz,$Mne,$Mg);}
...
if(lc('f'))Way($Mem);Waz();if(headers_sent())Wu($Mem);Whk($Mem);Wz($Mem);return;}
...
$Meq=Wey('bl',$Mce);if($Meq)return true;if($Mv){global$Mag,$Man;$Mer=$Man;$Man=DIR_CACHE."lightning/alpha";if(empty($_SERVER["HTTP_REFERER"]))return true;$Mbe=Wk($_SERVER["HTTP_REFERER"]);$Mbt=Wae($Mag.$Mbe);if(!$Mbt&&!strpos($Mbe,"index.php")&&(($Maf=strpos($Mbe,'?'))||($Maf=strpos($Mbe,'&')))){$Mbe=substr($Mbe,0,$Maf);$Mbt=Wae($Mag.$Mbe);}
$Man=$Mer;if(!$Mbt)return true;}
return false;}
...
function Wa_($Mfd){global$Mah,$Mw,$Mn,$Ma,$Mv,$Mq;if(!$Mah)return false;if(!empty(W_("request")->get["route"]))$Mce=W_("request")->get["route"];else$Mce="common/home";if(strlen(trim($Mfd))>6){if(strpos($Mfd,"<b>Fatal error<"))return false;if(strpos($Mfd,"YOU ARE BLOCKED ON THIS SERVER"))return false;if(!$Mv){if(!empty($Mq["compare"])and$Mn['ap']){if(!We($Mw))return false;Ww($Mn['ap'],"0",$Mfd);}
...
$Mfe=Wba();if($Mfe!=$Ma['aa']&&lc('s')){if(!We($Mw))return false;if($Mn['af']){$May=$Mn['ag'][$Ma['aa']];Wv('',"http".(($_SERVER["SERVER_PORT"]==443)?"s://":"://")."$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]",'name="redirect" value="','"',$May);Wx($Mfe,$Ma['aa'],$Mfd);Ww($Mn['af'],$May,$Mfd);}}}
$Mff=gzencode($Mfd,9);}else$Mff=false;global$Man,$Mfg;$Mfg=lc('l')*60;if($Mv)$Man=DIR_CACHE."lightning/tetha";else$Man=DIR_CACHE."lightning/alpha";if(!file_exists(DIR_CACHE."lightning/alpha"."/meters")){global$Mir;if($Mir)$Mir->query("TRUNCATE TABLE ".DB_PREFIX."lightning_product_to_page");}
if($Mce=="product/product"&&$Mff)$Mff.="`]".W_("request")->get["product_id"]."[`";Wbb($Mah,$Mff,true);if($Mce=="common/home")Wgc($Mfd);if($Mfd){global$Mfh;if($Mfh){global$Mir;$Mao=md5($Mah);$Mdb=substr($Mao,0,2)."/c".substr($Mao,2);$Mir->query("DELETE FROM ".DB_PREFIX."lightning_product_to_page WHERE page='".$Mdb."'");if(count($Mfh)>200){}else{$Mfi="INSERT INTO ".DB_PREFIX."lightning_product_to_page (product_id, page) VALUES ";$Mfj=true;foreach($Mfh as$Mfk=>$Maj)if($Mfk){if($Mfj)$Mfj=false;else$Mfi.=",";$Mfi.="(".$Mfk.", '".$Mdb."')";}
$Mir->query($Mfi);}}
if(lc('f')&&$_SERVER["REQUEST_METHOD"]!="POST")Way($Mfd);$Mfg=lc('l')*60;global$Mch,$Mk,$Mh;if(lc('n')and!$Mv and$Mch){global$My;$Mbz=Wbc("href='","'",str_replace('"',"'",$Mfd));$Mbz=array_unique($Mbz);foreach($Mbz as$Mcb=>$Mbe){$Mbe=str_replace("&amp;","&",$Mbe);if($Mbe==$Mk){unset($Mbz[$Mcb]);continue;}
...
$Man=DIR_CACHE."lightning/gamma";if($Mce!="common/home"){$Mfl=Wbd(Wm($Mh));if($Mfl)$Mbz=array_diff($Mbz,unserialize($Mfl));}
global$Ml;if(!in_array($Mk,$Mbz))$Mbz=array_merge(array($Mk),$Mbz);$Mbq=serialize(array_values($Mbz));$Mfm=Wbd($Mah);if(strlen($Mbq)*2>strlen($Mfm)){Wbb($Mah,$Mbq,true);}
Wbe($Mah,$Ml);$Man=DIR_CACHE."lightning/alpha";global$Maq,$Mbr,$Mtf;if($Maq and!$Maq['Mbz']and!$Mtf){Wp($Mbr);Wr("gen");}}}
if($Mn)return false;else return$Mff;}
function Wbf($Mda){return(int)(abs($Mda/10));}
function Wgc(&$Mfd){if(Wgk()=='ex')return;$Muo=array();preg_match_all('/\.(jpe?g|png)/i',$Mfd,$Mcu,PREG_OFFSET_CAPTURE);$Mup="'\"([{<";for($Mcb=0;$Mcb<strlen($Mup);$Mcb++)$Muq[$Mup[$Mcb]]=1;foreach($Mcu[0]as$Mbp){$Maf=$Mbp[1];$Mei=$Maf-1;while($Mei>0&&empty($Muq[$Mfd[$Mei]]))$Mei--;$Maf+=strlen($Mbp[0])-1;$Muo[]=substr($Mfd,$Mei+1,$Maf-$Mei);}
...
$Mdg=substr($Mdg,strlen(DIR_SYSTEM)-7);Whm($Mvm,substr(DIR_SYSTEM,0,-7).light_image_to_webp($Mdg,true));}
Waq($Mws);}
