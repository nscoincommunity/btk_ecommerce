<?php
class ModelExtensionShippingStarMotorCc extends Model {
    ...
    if(!empty($address['sub_district_id'])) {
        $costs = $this->cart->getCosts($this->config->get('module_bundle_city_id'),$address['sub_district_id'],$berat,"MOTOR 150 - 250 CC","STAR");
        ...
        $quote_data['star_motorcc'] = array(
            'code'         => 'star_motorcc.star_motorcc',
            ...
            'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('module_bundle_tax_class_id_star'), $this->config->get('config_tax')), $this->session->data['currency'])
      		...
      		$method_data = array(
        		...
        		'quote'      => $quote_data,
				'sort_order' => $this->config->get('shipping_star_motorcc_sort_order'),
        		'error'      => false
      		);
		}
