<?php
class ModelExtensionShippingStarExpress extends Model {
    ...
    if(!empty($address['sub_district_id'])) {
        $costs = $this->cart->getCosts($this->config->get('module_bundle_city_id'),$address['sub_district_id'],$berat,"Express","STAR");
        ...
        $quote_data['star_express'] = array(
            'code'         => 'star_express.star_express',
            ...
            'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('module_bundle_tax_class_id_star'), $this->config->get('config_tax')), $this->session->data['currency'])
      		...
      		$method_data = array(
        		...
        		'quote'      => $quote_data,
				'sort_order' => $this->config->get('shipping_star_express_sort_order'),
        		'error'      => false
      		);
		}
