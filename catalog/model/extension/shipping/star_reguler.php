<?php
class ModelExtensionShippingStarReguler extends Model {
    ...
    if(!empty($address['sub_district_id'])) {
        $costs = $this->cart->getCosts($this->config->get('module_bundle_city_id'),$address['sub_district_id'],$berat,"Reguler","STAR");
        ...
        $quote_data['star_reg'] = array(
            'code'         => 'star_reg.star_reg',
            ...
            'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('module_bundle_tax_class_id_star'), $this->config->get('config_tax')), $this->session->data['currency'])
      		...
      		$method_data = array(
        		...
        		'quote'      => $quote_data,
				'sort_order' => $this->config->get('shipping_star_reg_sort_order'),
        		'error'      => false
      		);
		}
