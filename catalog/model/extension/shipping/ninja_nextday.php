<?php
class ModelExtensionShippingNinjaNextday extends Model {
   ...
   if($unit == 'g') {
    ...
    if(!empty($address['sub_district_id'])) {
        $costs = $this->cart->getCosts($this->config->get('module_bundle_sub_district_id'),$address['sub_district_id'],$berat,"NEXTDAY","NINJA");
        ...
        $quote_data['ninja_nextday'] = array(
            'code'         => 'ninja_nextday.ninja_nextday',
            ...
            'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('module_bundle_tax_class_id_ninja'), $this->config->get('config_tax')), $this->session->data['currency'])
      		...
      		$method_data = array(
        		...
        		'quote'      => $quote_data,
				'sort_order' => $this->config->get('shipping_ninja_nextday_sort_order'),
        		'error'      => false
      		);
		}
